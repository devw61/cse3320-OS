...
void create_prg_asm_file(FILE *f_ptr, uint32_t addr, uint32_t end,
                         cartridge *cart, mapper_type *mapper, char *filename,
                         output_type o_type);
void create_prg_html_file(FILE *f_ptr, uint32_t addr, uint32_t end,
                          cartridge *cart, mapper_type *mapper, char *filename);
...
int main(int argc, char *argv[]) {
  ...
  if (o_type != HTML) {
    ...
    create_prg_asm_file(prg_f, addr, prg_end, cart, mapper,
                        parse_arg_filename(argv[1]), o_type);
  } else {
    sprintf(filename, "%s/%s_prg.html", dir, parse_arg_filename(argv[1]));
    prg_f = fopen(filename, "w");

    create_prg_html_file(prg_f, addr, prg_end, cart, mapper,
...
void create_prg_html_file(FILE *f_ptr, uint32_t addr, uint32_t end,
                          cartridge *cart, mapper_type *mapper,
                          char *filename) {
  // meta data of file
  char *html_header =
...
char *parse_arg_filename(char *file_path) {
  char *fname = (char *)malloc(sizeof(*file_path) / sizeof(char));
  int fname_ptr = 0;
  // start after ROMs/
